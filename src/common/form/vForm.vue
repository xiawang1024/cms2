    ...
    <el-form v-if="formModel" ref="vForm" :model="formModel" :rules="formRules" :label-width="labelWidth" :size="size">
      <div v-for="(section, sectionIndex) in formSettings" :key="sectionIndex" class="form-section">
        ...
        <div v-if="!section.slot" class="section-content">
          <el-form-item v-for="(item, index) in section.items" v-show="!item.hidden" :key="index" :label="item.label" :prop="item.name">
            <!-- 文字输入 -->
            <!--
              append: 文本框后置内容
             -->
            ...
            <template v-if="item.type=='carNumber'">
              <el-input v-model="formModel[item.name]" :placeholder="item.placeholder" :disabled="item.disabled">
                <el-select slot="prepend" v-model="formModel[item.name + '_carNumber']" placeholder="请选择" style="width:60px">
                  <el-option v-for="(province, pIndex) in provinceList" :label="province" :value="province" :key="pIndex"/>
                </el-select>
              </el-input>
            </template>
            <!-- 密码 -->
            <template v-else-if="item.type=='password'">
              <el-input v-model="formModel[item.name]" :placeholder="item.placeholder" :disabled="item.disabled" type="password"/>
            </template>
            <!-- 数字类型 -->
            <!--
              controls: 是否显示增减按钮
            ...
            <template v-else-if="item.type=='select'">
              <el-select v-model="formModel[item.name]" :placeholder="item.placeholder" :clearable="true" :disabled="item.disabled" :multiple="!!item.multiple" @change="(item.events&&item.events.change)?$emit(item.events.change, formModel[item.name]):null">
            ...
            <template v-else-if="item.type=='cascader'">
              <el-cascader :options="item.options" v-model="formModel[item.name]" :clearable="true" :props="item.props" expand-trigger="hover"/>
            </template>
            <!-- 时间日期范围选择 -->
            <template v-else-if="item.type=='datetimerange'">
              <el-date-picker v-model="formModel[item.name]" :default-time="item.defaultTime" type="datetimerange" align="right" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"/>
            </template>
            <!-- 日期范围选择 -->
            ...
            <template v-else-if="item.type=='date'">
              <el-date-picker v-model="formModel[item.name]" :placeholder="item.placeholder || '选择日期'" type="date"/>
            </template>
            <!-- 日期和时间选择 -->
            <template v-else-if="item.type=='datetime'">
              <el-date-picker v-model="formModel[item.name]" :placeholder="item.placeholder || '选择日期'" type="datetime"/>
            </template>
            <!-- 时间范围选择 -->
            <template v-else-if="item.type=='timerange'">
              <el-time-picker v-model="formModel[item.name]" :placeholder="item.placeholder || '选择时间范围'" is-range range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间"/>
            </template>
            <!-- 时间选择 -->
            ...
            <template v-else-if="item.type=='radio'">
              <el-radio-group v-model="formModel[item.name]" @change="(item.events&&item.events.change)?$emit(item.events.change, formModel[item.name]):null">
                <el-radio v-for="(radioItem, radioIndex) in item.options" :disabled="item.disabled" :key="radioIndex" :label="radioItem.value">{{ radioItem.label }}</el-radio>
              </el-radio-group>
            </template>
            <!-- 多选 -->
            ...
            <template v-else-if="item.type=='textarea'">
              <el-input :autosize="{ minRows: 2, maxRows: 4}" :placeholder="item.placeholder || '请输入内容'" :maxlength="item.maxlength" v-model="formModel[item.name]" type="textarea"/>
            </template>
            <!-- 图片上传 -->
            <!--
              limit: 图片上传数量限制
            ...
            <template v-else-if="item.type=='remoteCity'">
              <website-select v-model="formModel[item.name]" :disabled="item.disabled" :is-show-all="item.isShowAll" :integrated-value="!!item.integratedValue" type="city" holder="请输入城市" @change="(item.events&&item.events.change)?$emit(item.events.change):null"/>
            </template>
