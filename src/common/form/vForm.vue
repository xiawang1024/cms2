        ...
        >
          ...
          >
            ...
            <template v-else-if="item.type=='textarea'">
              <el-input :autosize="{ minRows: 4, maxRows: 8}" :placeholder="item.placeholder || '请输入内容'" :disabled="item.disabled" :maxlength="item.maxlength" v-model="formModel[item.name]" :show-word-limit = "item.showWordLimit" type="textarea"/>
            ...
            <template v-else-if="item.type=='img'">
              ...
              >
                ...
                >
                  ...
                  <!-- 图片描述输入框 -->
                  <div
                    v-if="item.hasTextInput && imgUploadText[item.name].length"
                    class="upload-text-container"
                  >
                    <div
                      class="imageHandel"
                      v-for="(uploadTextItem, uploadTextIndex) in imgUploadText[item.name]"
                      :key="uploadTextIndex"
                    >
                      <div class="handelBtn">
                        <!-- <span>设为封面</span>
                        ...
                        <el-button
                          ...
                          @click="setCover(imgUploadText[item.name])"
                        >设置封面</el-button>
            ...
            <template v-else-if="item.type=='remoteCity'">
              <website-select
                ...
                :integrated-value="!!item.integratedValue"
                type="city"
                holder="请输入城市"
        ...
        settingItem.items.forEach(item => {
          if (!item.hidden && item.required) {
            ...
            tmpRules[item.name] = [
              {
                required: true,
                message: item.defineMessage
                  ? ruleMsg + item.defineMessage
                  : ruleMsg + item.label, // 自定义校验提示
                trigger: "change"
              }
            ];
            // if (item.type == 'date') {
      ...
      if (this.flatFormSettings) {
        Object.keys(this.flatFormSettings).forEach(item => {
          ...
          if (formItem.type == "img" && formItem.hasTextInput) {
            data[formItem.name + "UploadText"] = this.imgUploadText[
              formItem.name
            ];
          }
        });
