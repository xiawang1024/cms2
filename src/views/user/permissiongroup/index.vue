<template>
  <div class="components-container">
        ...
        <span slot-scope="{ node, data }" class="custom-tree-node">
          ...
          <span v-if="treeButtonFlag">
            ...
            <el-button
              type="text"
              size="mini"
              @click="() => updatePermissionManagerDialog(node, data)"
            >权限</el-button>
          </span>
        </span>
      </el-tree>
    ...
    >
      <el-form-renderer
        ref="dialogFormRender"
        :inline="dialogFormInlineFlag"
        :class="dialogFormInlineFlag ? 'demo-form-inline' : '' "
        :content="dialogForm"
        label-width="100px"
      >
        <el-form-item label="上级权限组">
          <el-cascader
            v-model="dialogPermissionGroupOptions"
            :options="treeData"
            :props="cascaderProps"
            clearable
            change-on-select
    ...
    <!-- 权限分配管理 弹窗组件 -->
    <el-dialog
      :visible.sync="dialogPermissionManagerVisible"
      top="8vh"
      width="70%"
      :before-close="handleClose"
      title="权限分配管理"
    >
      <div>
        <permission
          ...
          :use-props-btn="hideBtn"
          :button-array-props="dialogPermissionManagerButtonArrayProps"
          :import-button-flag="dialogPermissionManagerImportButtonFlag"
        >
...
import {
  ...
  PermissionGroupPermissionRelSave,
  PermissionGroupPermissionRelPermissionInfoByGroupId
} from "@/api/user/permissiongroup";
  ...
  methods: {
    ...
    updateTreeNodeDialog(node, data) {
      ...
      _this.$nextTick(function() {
        ...
        _this.dialogPermissionGroupOptions =
          data.parentPermissionGroupIds !== null &&
          data.parentPermissionGroupIds !== undefined
            ? data.parentPermissionGroupIds.split(",")
            : [];
      });
    },
    updatePermissionManagerDialog(node, data) {
      ...
      _this.$nextTick(function() {
        // 查询出来权限组下属的权限信息
        PermissionGroupPermissionRelPermissionInfoByGroupId(
          _this.dialogPermissionManagerPermissionGroupId
        )
          .then(async res => {
            console.log(res);
            _this.$refs.dialogPermissionRef.$refs["permission-table"].$refs[
              "pap-base-table"
            ].clearSelection();
    ...
    dialogFormSubmit() {
      ...
      const permissionGroupLastId = this.dialogPermissionGroupOptions[
        this.dialogPermissionGroupOptions.length - 1
      ];
      dialogFormObj.parentPermissionGroupId = permissionGroupLastId;
      return new Promise((resolve, reject) => {
        // 开始请求
        if (_this.dialogSubmitUrlType === "CREATE") {
          PermissionGroupCreate(dialogFormObj)
            .then(async res => {
              console.log(res);
              _this.getList();
        ...
        if (_this.dialogSubmitUrlType === "UPDATE") {
          PermissionGroupUpdate(dialogFormObj)
            .then(async res => {
              console.log(res);
              _this.getList();
              _this.clearDialogForm();
    ...
    dialogPermissionManagerSubmit() {
      ...
      return new Promise((resolve, reject) => {
        PermissionGroupPermissionRelSave(obj)
          .then(async res => {
            console.log(res);
            const h = this.$createElement;
            if (res.data.code == 0) {
