      ...
      <span slot="customerFormItem">
        <el-form-item label="所属应用">
          <el-select
            ...
            :remote-method="searchApplicationInfo"
            filterable
            remote
            reserve-keyword
            placeholder="请选择所属应用"
        ...
        <el-form-item label="请求协议">
          <el-select
            v-model="dialogHttpMethod"
            :remote-method="searchOperationInfo"
            filterable
            remote
            reserve-keyword
            placeholder="请选择请求协议"
          >
            <el-option
              ...
              :key="item.sysOperationId"
              :label="item.sysOperationName"
              :value="item.sysOperationId"
            />
          </el-select>
        </el-form-item>
      </span>
  ...
  methods: {
    ...
    searchOperationInfo(queryName) {
      ...
      return new Promise((resolve, reject) => {
        // 开始请求
        OperationList(searchFormObj, 1, 10)
          .then(async res => {
    ...
    searchApplicationInfo(queryName) {
      ...
      return new Promise((resolve, reject) => {
        // 开始请求
        ApplicationList(searchFormObj, 1, 10)
          .then(async res => {
    ...
    multipleSelectionEmit(val) {
      ...
      if (this.multipleSelection.length !== 1) {
        ...
        if (
          this.buttonArray.length > 1 &&
          this.buttonArray[1].name === "编辑"
        ) {
    ...
    dialogSubmitClickCallback(type, res, contextThis) {
      ...
      if (res.data.code == 0) {
        ...
        contextThis.getList();
        // 查询成功之后，清空表格的选中值
        contextThis.$refs["permission-table"].$refs[
          "pap-base-table"
        ].clearSelection();
      } else {
        contextThis.$notify({
