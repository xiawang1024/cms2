<template>
  <div style="height: 100%">
    ...
    <el-dialog
      class="binding"
      title="Google二次验证绑定"
      :visible.sync="dialogGoogleBindingVisible"
      width="30%"
    >
      <span>
        <img :src="dialogGoogleBindingQrCodeBase64" alt="二维码图片" >
      </span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogGoogleBindingVisible = false">取 消</el-button>
        <el-button
          type="primary"
          @keyup.enter.native="dialogGoogleBindingHandle"
          @click.native.prevent="dialogGoogleBindingHandle"
        >确 定</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog
      class="doublecheck"
      title="Google二次验证(请查看APP中的动态口令)"
      :visible.sync="dialogGoogleDoubleCheckVisible"
      width="30%"
    >
      <el-form label-width="80px" @submit.native.prevent>
        <el-form-item label="动态口令" style="background: none">
          <el-input
            v-model="dialogGoogleInputSecondCheckkey"
            ref="dialogGoogleInputSecondCheckkey"
            style="border: none;"
            class="input-32"
          />
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          @keyup.enter.native="dialogGoogleDoubleCheckHandle"
          @click.native.prevent="dialogGoogleDoubleCheckHandle"
          size="small"
        >确 定</el-button>
      </span>
    </el-dialog>
...
<script>
import {
  UserNoAuthCheckUserInfo,
  UserNoAuthQrCode,
  UserNoAuthQrCodeBinding,
  UserNoAuthGoogleDoubleCheck
} from "@/api/user/user";
export default {
  name: "Login",
  data() {
    ...
    handleGoogleQrCode() {
      ...
      return new Promise((resolve, reject) => {
        ...
        UserNoAuthQrCode(
          _this.loginForm.username,
          _this.loginForm.password,
          _this.dialogGoogleBindingSecretKey
        )
          .then(async res => {
            console.log(res.data);
            if (res.data.code == 0) {
              _this.dialogGoogleBindingQrCodeBase64 =
                "data:img/jpg;base64," + res.data.result;
            }
            // 结束
            resolve();
          })
    ...
    dialogGoogleDoubleCheckHandle() {
      ...
      return new Promise((resolve, reject) => {
        ...
        UserNoAuthGoogleDoubleCheck(
          _this.loginForm.username,
          _this.loginForm.password,
          _this.dialogGoogleInputSecondCheckkey
        )
          .then(async res => {
            console.log(res.data);
            if (res.data.code == 0) {
