  ...
  <div class="column-manage">
    ...
    <div class="tool-bar">
      <el-button
        ...
        @click="columnAddEdit(true, {},'添加一级栏目')"
        size="mini"
      >添加一级栏目</el-button>
    </div>
    <el-table :data="tableData" highlight-current-row style="width: 100%">
      <el-table-column prop="channelName" label="栏目名称" min-width="250" show-overflow-tooltip>
        <template slot-scope="scope">
          <div>
            <span
              v-for="(group) in channelNameChange(scope.row.parentChannelNames)"
              :key="group"
              class="space-length"
            />
            <span
              v-for="(ele, index) in channelNameChange(scope.row.parentChannelNames)"
              :key="index"
              class="space-holder"
            />
      ...
      <el-table-column label="设置" width="250">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            @click="columnTemplate(scope.row)"
            v-if="checkAuth('cms:channel:template')"
          >栏目模板</el-button>
          <el-button
            type="text"
            size="small"
            @click="extendsWord(scope.row)"
            v-if="checkAuth('cms:channel:extendsWord')"
          >扩展字段</el-button>
          <el-button
            type="text"
            size="small"
            @click="tagSetting(scope.row)"
            v-if="checkAuth('cms:channel:tagSetting')"
          >标签设置</el-button>
          <el-button
            type="text"
            size="small"
            @click="waterSetting(scope.row)"
            v-if="checkAuth('cms:channel:waterSetting')"
          >水印设置</el-button>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            v-if="checkAuth('cms:channel:edit')"
            @click="columnAddEdit(false, scope.row,'栏目修改')"
          >修改</el-button>
          <el-button
            type="text"
            size="small"
          ...
          <el-button
            type="text"
            size="small"
            @click="columnAddEdit(true, scope.row,'添加子级栏目')"
            v-if="checkAuth('cms:channel:add')"
          >添加子级栏目</el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    columnAddEdit(handelType, row, column) {
