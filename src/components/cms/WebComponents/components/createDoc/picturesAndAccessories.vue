<template>
  <div class="document-upload-file">
              ...
              <template slot="information">
                <div class="file-infor">
                  ...
                  <div class="desc">
                    ...
                    >日期：{{ parseInt(filedetail.createTime)|timeFilter }}</div>
    ...
    <div class="upload-btn">
      ...
      <el-button
        :disabled="Boolean(contextMenu.docId) && (docInformation.articleStatus ==1) && (baseInfor.userName !== docInformation.createUser)"
        type="primary"
        size="mini"
        @click="save('0')"
      >保存</el-button>
...
export default {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        documentInfor(id)
          .then(response => {
            ...
            _this.formData.contentVideosList = _this.differenceFile(
              response.data.result.articleAttachmentsList,
              "VIDEO"
            );
            _this.formData.contentImagesList = _this.differenceFile(
              response.data.result.articleAttachmentsList,
              "IMG"
            );
            _this.formData.contentAudioList = _this.differenceFile(
              response.data.result.articleAttachmentsList,
              "AUDIO"
            );
            _this.formData.articleAttachmentsList = _this.differenceFile(
              response.data.result.articleAttachmentsList,
              "OTHER"
            );
    ...
    getSubmitData() {
      ...
      let videoFile = this.addCategory(
        this.$refs.imageForm.formModel.contentVideosList,
        "VIDEO"
      );
      let audioFile = this.addCategory(
        this.$refs.imageForm.formModel.contentAudioList,
        "AUDIO"
      );
      let otherFile = this.addCategory(
        this.$refs.imageForm.formModel.articleAttachmentsList,
        "OTHER"
      );
      let articleAttachmentsListCombine = articleAttachmentsList.concat(
        videoFile,
        audioFile,
        otherFile
      );
        ...
        if (
          ...
          for (let i = 0; i < resoultObj.extFieldsList.length; i++) {
            if (
              ...
              !resoultObj.extFieldsList[i].fieldValue
            ) {
              this.$message.warning(
                `${resoultObj.extFieldsList[i].label}不能为空`
              );
              return;
            }
          }
