          ...
          <el-col :span="5">
            ...
            <div class="article-content-right">
              <div class="other">
                ...
                <div>发布时间：{{ documentInfor.publishTime }}</div>
                ...
                <div v-if="documentInfor.extFieldsList && documentInfor.extFieldsList.length">
                  <div
                    v-for="(ele, index) in documentInfor.extFieldsList"
                    :key="index"
                  >{{ ele.label }}:{{ ele.fieldValue }}</div>
                </div>
              </div>
            </div>
          </el-col>
      ...
      return new Promise((resolve, reject) => {
        documentInfor(id)
          .then(response => {
            ...
            this.imageList = this.differenceFile(
              response.data.result.articleAttachmentsList,
              "IMG"
            );
            this.videolist = this.differenceFile(
              response.data.result.articleAttachmentsList,
              "VIDEO"
            );
            this.audioList = this.differenceFile(
              response.data.result.articleAttachmentsList,
              "AUDIO"
            );
